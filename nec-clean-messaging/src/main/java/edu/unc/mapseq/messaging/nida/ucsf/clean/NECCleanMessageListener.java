...
public class NECCleanMessageListener implements MessageListener {

    private final Logger logger = LoggerFactory.getLogger(NECCleanMessageListener.class);

    private WorkflowBeanService workflowBeanService;

    public NECCleanMessageListener() {
        super();
    }
                ...
                if (entityJSONObject.has("entity_type")) {
                    ...
                    if (WorkflowRun.class.getSimpleName().equals(entityType)) {
                        workflowRun = EntityUtil.getWorkflowRun(workflowBeanService.getMaPSeqDAOBean(),
                                "NECClean", entityJSONObject, account);
                    }
